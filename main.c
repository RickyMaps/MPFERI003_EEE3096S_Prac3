...
uint8_t bit_array[] = {0b10101010, 0b01010101, 0b11001100, 0b00110011, 0b11110000, 0b00001111};
...
int main(void)
{
  ...
  for (int i = 0; i <6;i++){
	  write_to_address(i,bit_array[i]);
  }
...
void EXTI0_1_IRQHandler(void)
{
    ...
    if(__HAL_TIM_GET_AUTORELOAD(&htim6) == 999){
		
		  __HAL_TIM_SET_AUTORELOAD(&htim6, 500 - 1);  //  set frequency to 2Hz
		     lastTick = HAL_GetTick();

			  }
    
		if (__HAL_TIM_GET_AUTORELOAD(&htim6) == 499){
		  __HAL_TIM_SET_AUTORELOAD(&htim6, 1000 - 1);  // set frequency to 1Hz 
		    lastTick = HAL_GetTick();
...
void TIM16_IRQHandler(void)
{
	...
	uint16_t EEPROM_value = read_from_address(eeprom_address);

	// TODO: Change LED pattern; output 0x01 if the read SPI data is incorrect
	
	
	    if (EEPROM_value == bit_array[eeprom_address])
	    {
	        char lcdLine[16];
	        sprintf(lcdLine, "%d", EEPROM_value);
	        writeLCD(lcdLine);
	    }
	    else
	    {
	    ...
	    eeprom_address = (eeprom_address + 1) % 6;
}
